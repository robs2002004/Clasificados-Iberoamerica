...
const PLANES_POR_DEFECTO = {
  basico: { precio: 0, dias: 15 },
  destacado: { precio: 3.99, dias: 30 },
  premium: { precio: 9.99, dias: 60 }
};
...
exports.crearSesionPago = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const dias = planInfo ? planInfo.dias : undefined;
      const precio = planInfo ? planInfo.precio : undefined;
      if (!planInfo || !dias || precio === undefined || precio === null) {
...
exports.capturarPago = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const precios = await obtenerPrecios();
      const dias = (precios[plan] && precios[plan].dias) || 30;
